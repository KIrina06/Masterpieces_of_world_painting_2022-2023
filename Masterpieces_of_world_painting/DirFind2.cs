...
{
    public partial class DirFind2 : Form
    {
        public DirFind2()
        {
            {
        ...
        private void DirFind_Load(object sender, EventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                sql = @"SELECT d.Name_direction, c.Name_country, d.Time_of_appearance
                        FROM dbo.Directions AS d JOIN dbo.Countries AS c ON d.Id_country = c.Id_country
                        WHERE d.Name_direction LIKE '" + mynum + "%'"; 
            }
            else
            {
                sql = @"SELECT d.Name_direction, c.Name_country, d.Time_of_appearance
                        FROM dbo.Directions AS d JOIN dbo.Countries AS c ON d.Id_country = c.Id_country
                        WHERE d.Name_direction LIKE '" + mynum + "%' AND d.Id_country = (SELECT Id_country FROM dbo.Countries WHERE Name_country = '" + comboBox1.Text + "')";
            }
